...
登入註冊頁面採用簡潔的卡片式設計，延續中國風配色風格。系統使用 NextAuth.js 實現身份驗證，支援電子郵件與密碼登入，並提供「訪客登入」功能讓用戶無需註冊即可體驗核心功能。密碼採用 bcryptjs 加密儲存，確保用戶憑證安全。
...
#figure(
  image("UiPicture\aiQueryPage\aiQuery01.jpg", width: 100%),
  caption: [AI問答功能介面：初始回答]
)

#figure(
  image("UiPicture\aiQueryPage\aiQuery02.jpg", width: 80%),
...
紅學社是系統的社群討論功能，採用信息流式卡片佈局。頁面頂部提供發文表單與搜尋框，下方以時間序展示所有貼文，每則貼文顯示作者、時間戳、內容、標籤，並整合按讚與評論功能。評論支援無限層級的巢狀回覆結構。系統整合自動內容過濾服務，即時檢測不當內容、髒話與個資洩露，確保社群討論環境的安全與品質。用戶亦可將閱讀筆記一鍵分享至紅學社，促進學習心得交流。
