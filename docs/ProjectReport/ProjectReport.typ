...
#page(header: none, numbering: none)[
  ...
  #align(center)[
    ...
    #v(0.5cm)
    #text(size: 20pt)[#department 畢業專題競賽]
    #v(0.5cm)
    #text(size: 20pt)[#doc_type]
    #v(3cm)
...
// 設定四級標題樣式，四級標題上方空隙0.1em，標題下方空隙縮小
#show heading.where(level: 4): it => {
  set text(size: 12pt, weight: "bold")
  it
  // 在四級標題後添加重置段落狀態的機制，並縮小下方間距
  v(-0.8em)
  par()[#text(size: 0pt)[#h(0pt)]]
}
  ...
  if it.caption != none {
    align(center)[
      #box(
        ...
        inset: (x: 6pt, y: 3pt),
        radius: 2pt,
        text(fill: white, size: 12pt)[#it.caption]
      )
    ]
    v(0.1em)
  }
...
(一) 動機維持系統：遊戲化學習設計
針對持續動機問題，系統採用遊戲化設計將閱讀轉化為成就解鎖體驗。建立「閱讀成就徽章系統」，設置「初窺門第」、「人物通曉」、「詩詞雅趣」等里程碑，配備精美視覺與儀式感。「進度視覺化追蹤」透過動態環形進度條、學習熱力圖等元素，讓用戶清楚掌握學習狀態。
...
基於梁寧《產品思維課程》的用戶需求分析框架，古典文學閱讀中的用戶需求分為三個層次。核心痛點是持續學習動力缺失，表現為「害怕再次半途而廢」的恐懼心理，具體包括缺乏明確學習里程碑與成就感回饋機制、無法有效追蹤學習進度與掌握程度、缺少外部激勵與持續動力維持機制，以及對長篇古典文學的時間投入產生焦慮。

重要痛點是理解障礙與文化隔閡，源於語言的時代隔閡、文化背景的斷層、人物關係的複雜性，以及缺乏專業解讀的引導。重要爽點體現在理解突破的即時滿足，包括閱讀頁面AI智慧問答的快速回應、視覺化章回知識圖譜的清晰認知，以及社群討論區域功能帶來的互動與支持。輔助癢點則是文化身份認同需求，滿足用戶對文化素養提升與社會認同的渴望。

根據需求優先級分析，核心功能需求包括動機維持系統（閱讀成就徽章系統、學習遊戲化設計）和理解輔助工具（AI智慧問答模組、互動式章回知識圖譜、社群互動功能模組）。

== 市場與競爭者分析

古典文學閱讀市場呈現多元化的解決方案格局，現有競爭者可分為三大類別(參下頁之表格)：
...
#figure(
  table(
    ...
    [*AI系統性融合的技術創新*],
    [基於AI的章節知識圖譜自動生成系統，智能解析文本結構與人物關係。],
    
    [*專業內容資源整合*],
    [建立章回知識圖譜的完整體系，提供系統性古典文學專家解讀。],
...
介紹頁採用傳統中國紅與雅致金色的主視覺設計，頂部導航欄提供「主頁」、「特色」、「主要功能」等區塊的快速入口，並支援繁體中文、簡體中文與英文三語即時切換。頁面以卡片形式呈現讀者痛點與系統解決方案，底部設有醒目的「免費註冊」行動呼籲按鈕，引導用戶開始使用。
...
登入註冊頁面採用簡潔的卡片式設計，延續中國風配色風格。系統使用 NextAuth.js 實現身份驗證，支援電子郵件與密碼登入，並提供「訪客登入」功能讓用戶無需註冊即可體驗核心功能。
...
學習總覽首頁（Dashboard）採用卡片式佈局，提供用戶即時的學習狀態總覽。頁面上方以動態環形進度條展示章回完成度，並顯示當前等級、經驗值與連續學習天數等遊戲化指標。下方提供「閱讀」、「每日任務」、「紅學社」、「成就」等核心功能的快捷入口卡片，方便用戶快速導航至各功能模組。

==== 閱讀頁面

#figure(
  image("UiPicture/ReadingPage/readingPage_White_JPG.jpg", width: 100%),
  caption: [閱讀頁面(預設白色主題)]
)


#figure(
  image("UiPicture/ReadingPage/readNote02.jpg", width: 90%),
  caption: [閱讀頁面：寫筆記]
)

#figure(
  image("UiPicture/ReadingPage/readNote01.jpg", width: 90%),
  caption: [閱讀頁面：閱覽筆記]
)

閱讀頁面是系統的核心功能，提供沉浸式的《紅樓夢》研讀環境。頁面支援多種閱讀主題（白色、米黃、護眼綠、夜間模式）、字體選擇（含思源宋體）、字號調整，以及單欄、雙欄、三欄的版面配置切換。用戶選取文本後可觸發快捷操作欄，提供「寫筆記」、「複製」、「問 AI」等功能。側邊面板整合筆記管理、章節目錄與知識圖譜視覺化功能，支援在閱讀過程中即時記錄心得與查看人物關係。

==== AI 問答功能

#figure(
  image("UiPicture/aiQueryPage/aiQuery01.jpg", width: 70%),
  caption: [AI問答功能介面：初始回答]
)

#figure(
  image("UiPicture\aiQueryPage\aiQuery02.jpg", width: 75%),
  caption: [AI問答功能介面：正式回答]
)

AI 問答功能整合 Perplexity Sonar API，提供具備網路搜尋能力的智能問答服務。用戶在閱讀頁面選取文本後點擊「問 AI」即可發起提問，系統以串流方式（Server-Sent Events）即時顯示回答內容。介面包含「思考過程」指示器展示 AI 推理步驟，正式回答則以結構化格式呈現，並附帶學術文獻引用來源供用戶查證。
...
成就頁面採用卡片式佈局展示用戶的學習成果與遊戲化進度。頁面顯示已獲得的成就徽章（含專屬圖標、描述與獲得日期）、當前等級與經驗值進度條、連續學習天數統計，以及個人學習目標的完成情況追蹤。系統透過經驗值累積與等級提升機制激勵用戶持續學習，完成閱讀、任務與社群互動均可獲得對應經驗值獎勵。

==== 知識圖譜頁面

知識圖譜功能採用 D3.js 力導向圖（Force Simulation）演算法實現人物關係的互動式視覺化。節點類型包含人物、地點、事件與概念，連結類型涵蓋家族關係、友誼、衝突等，並以色彩編碼區分不同類別。用戶可透過縮放、平移與拖曳操作探索關係網絡，搜尋功能支援快速定位特定人物並以高亮效果標示。圖譜資料依章節動態載入，展示各章回的人物出場與情節關聯。
#figure(
  image("UiPicture\knowledgeGraphPage\knowledgeGraph01.jpg", width: 100%),
  caption: [知識圖譜頁面：章回知識圖譜]
)

#figure(
  image("UiPicture\knowledgeGraphPage\knowledgeGraph02.jpg", width: 100%),
  caption: [知識圖譜頁面：鼠標懸停角色顯示關係]
)

==== 每日任務頁面

#figure(
  image("UiPicture\dailyTask\dailyTask01.png", width: 100%),
  caption: [每日任務頁面：綜覽]
)

#figure(
  image("UiPicture\dailyTask\dailyTask02.png", width: 70%),
  caption: [每日任務頁面：任務頁面]
)

#figure(
  image("UiPicture\dailyTask\dailyTask03.png", width: 60%),
  caption: [每日任務頁面：AI批改畫面]
)

每日任務頁面是系統遊戲化學習的核心，提供閱讀理解測驗、詩詞品質評析、人物性格分析等多種任務類型。頁面上方顯示連續學習天數（Streak）與今日任務完成進度，下方以任務卡片列出待完成項目，標示任務類型、難度與經驗值獎勵。用戶提交答案後，系統整合 OpenAI GPT-5-mini 進行 AI 智能批改，即時顯示評分（0-100）與詳細回饋建議。

==== 紅學社：社群功能頁面
#figure(
  image("UiPicture/communityPage/communityPage.png", width: 100%),
  caption: [社群功能頁面]
)

紅學社是系統的社群討論功能，採用資訊流式卡片佈局。頁面頂部提供發文表單與搜尋框，下方以時間序展示所有貼文，每則貼文顯示作者、時間戳、內容、標籤，並整合按讚與評論功能。評論支援無限層級的巢狀回覆結構。系統整合自動內容過濾服務，即時檢測不當內容、髒話與個資洩露，確保社群討論環境的安全與品質。用戶亦可將閱讀筆記一鍵分享至紅學社，促進學習心得交流。

#pagebreak()

== 使用技術

「紅樓慧讀」系統採用現代化的全端架構設計，以 Next.js 15 作為核心開發框架，結合雙 AI 模型整合策略與雲端資料庫解決方案，打造高效能、易維護的古典文學學習平台。整體技術選型以開發效率、用戶體驗和成本效益為核心考量。
...
#figure(
  table(
    ...
    [前端框架], [Next.js 15 + React 18], [支援 SSR/SSG 提升載入效能與 SEO，App Router 簡化路由管理],
    [程式語言], [TypeScript], [靜態型別檢查降低錯誤率，提升程式碼可維護性],
    [UI 樣式], [Tailwind CSS + Shadcn UI], [原子化 CSS 快速開發一致性介面，支援響應式與暗色模式],
    [後端 API], [Next.js API Routes], [前後端整合的全端開發模式，簡化部署與維護],
    [身份驗證], [NextAuth.js], [支援 JWT 會話管理、訪客登入與密碼加密儲存],
    [資料庫], [Turso LibSQL], [基於 SQLite 的雲端資料庫，低延遲且運維成本低],
    [AI 問答], [Perplexity Sonar API], [具網路搜尋能力，提供具學術引用的即時回答],
    [AI 評分], [OpenAI GPT-5-mini], [穩定的 JSON 結構化輸出，適合教育評量與批改],
    [知識圖譜], [D3.js], [力導向圖演算法實現互動式人物關係視覺化],
    [測試框架], [Jest + Testing Library], [單元測試與元件測試確保程式碼品質],
  ),
  caption: [系統技術架構總覽],
  kind: table
)

在前端架構方面，Next.js 15 的 App Router 提供檔案式路由系統，搭配 React 18 的元件化開發模式，實現高效的頁面組織與狀態管理。Tailwind CSS 的原子化設計理念結合 Shadcn UI 元件庫，確保介面開發的一致性與無障礙支援。

後端採用 Next.js API Routes 實現 RESTful 介面，搭配 Repository Pattern 設計模式分離業務邏輯與資料存取。資料庫選用 Turso LibSQL 雲端服務，相較傳統關聯式資料庫大幅降低運維複雜度，同時保持 SQL 標準相容性。

AI 功能採用雙模型策略：Perplexity Sonar API 負責問答任務，其網路搜尋能力可整合學術研究成果提供具引用來源的回答；OpenAI GPT-5-mini 負責評分批改，穩定的結構化輸出適合教育評量場景。知識圖譜則運用 D3.js 力導向演算法，將章回人物關係轉化為可互動探索的視覺化網絡。

開發流程採用 TypeScript 嚴格模式確保型別安全，Jest 測試框架維持程式碼品質，並透過 Git 版本控制與 Vercel 平台實現持續整合部署。
...
在內容發展上，我們欲擴充名家解讀庫、引入用戶共創機制，鼓勵用戶貢獻高品質學習筆記與文學見解，進一步強化平台的社群價值與知識深度，實現「共學、共創、共讀」的社群學習目標。同時，我們將建立用戶個人品牌經營機制，讓有影響力的用戶能夠建立自己的學術聲譽和追蹤者社群。透過讓熱門多追蹤數的用戶帶領其餘小用戶，形成真正的社群效應，創造知識分享與傳播的正向循環，讓優質的學習內容和見解能夠在社群中自然傳播和累積。


= 參考文獻
1. 鄭煦 (2018), 高中紅樓夢整本書閱讀教學的設計與實施, 北京師範大學文學院.
2. Carlson, C. R. (2017). SRI's turnaround: how four principles guided high-value, marketcreating innovation. Strategy & Leadership, 45(3), 3-11. https://doi.org/10.1108/SL-052017-0052
3. 鄒欣 (2019), 構建之法：現代軟件工程 (第三版), 北京：人民郵電出版社.
4. SRI International. (n.d.). NABC Value Proposition. Retrieved from https://www.sri.com
5. Huang, H., Chen, C., Sheng, Z., Li, Y., & Zhang, W. (2024). Can LLMs be Good Graph Judge for Knowledge Graph Construction? arXiv preprint arXiv:2411.17388. https://doi.org/10.48550/arXiv.2411.17388
6. 廖俊雲 (2024), 元宇宙營銷：數字營銷新浪潮, 北京：北京大學出版社.
7. 黃勁松 (2024), 數字營銷學, 北京：機械工業出版社.
8. 楊雪 (2024), 電子商務：商業模式、戰略與運營, 北京：機械工業出版社.
9. 戴維·羅斯 (2016), 極致：互聯網時代的產品設計, 北京：中信出版社.
