...
首先，在系統的前端介面層，採用 Next.js 15 App Router 架構，以「首頁（Dashboard）」作為核心導航中樞。用戶可透過「訪客介紹頁面」了解系統功能後，再經由「登入註冊頁面」使用 NextAuth.js 進行身份驗證。成功登入後進入首頁，可選擇五大核心功能：「閱讀頁面」提供沉浸式閱讀體驗與AI問答功能、「每日任務頁面」提供遊戲化學習挑戰、「成就系統頁面」展示學習成果與激勵機制、「討論互動社群頁面」促進用戶間的討論交流，以及「人物關係圖譜頁面」呈現紅樓夢的人物關係網絡。

接下來，API服務層直接使用 Next.js 內建的 API Routes 機制，無需額外的 API Gateway。所有來自前端的請求都會經過對應的 API 路由進行處理，包括 `/api/auth/*` 處理認證、`/api/chapters/*` 提供章節內容、`/api/perplexity-qa-stream/*` 處理 AI 問答串流、`/api/daily-tasks/*` 管理每日任務等。這種設計簡化了部署複雜性，同時保持良好的關注點分離。

之後，後端服務層共包含九個核心服務模組：

1. *認證服務（auth-options.ts）*：使用 NextAuth.js 實現用戶身份驗證，支援 Credentials Provider 進行帳號密碼登入。

2. *閱讀內容服務（chapter-loader.ts）*：處理章節文本載入、白話文對照顯示，以及章節知識圖譜資料的讀取。

3. *AI問答服務（perplexity-client.ts）*：整合 Perplexity Sonar API，提供具備網路搜尋能力的智能問答功能，支援串流回應以提升用戶體驗。

4. *AI評分服務（openai-client.ts）*：整合 OpenAI GPT-5-mini，負責所有每日任務的評分與回饋生成，包括閱讀理解、詩詞賞析、人物分析等。

5. *每日任務服務（daily-task-service.ts）*：管理每日學習任務的完整生命週期，包括任務生成、提交評估、連續學習天數追蹤。

6. *成就系統服務（user-level-service.ts）*：管理用戶經驗值計算、等級提升、成就徽章解鎖與獎勵分發。

7. *社群互動服務（community-service.ts）*：支援貼文發布、留言互動、按讚功能，並整合內容過濾服務確保社群安全。

8. *筆記服務（notes-service.ts）*：提供筆記建立、標籤管理、公開/私人設定等功能，支援用戶建立個人學習知識庫。

9. *內容過濾服務（content-filter-service.ts）*：自動檢測不當內容、髒話、仇恨言論與個資洩露，確保平台內容品質。

所有後端服務都統一通過資料存取層（Repository Pattern）進行資料操作。資料存取層包含七個專門的 Repository 模組，分別處理用戶資料、社群貼文、留言、筆記、文字標記、閱讀進度與每日任務的 CRUD 操作。所有 Repository 都使用 Prepared Statements 防止 SQL 注入攻擊：

1. *user-repository*：管理用戶帳戶、等級與經驗值資訊
2. *community-repository*：管理社群貼文的發布與互動
3. *comment-repository*：處理貼文留言與回覆
4. *note-repository*：管理用戶筆記內容與標籤
5. *highlight-repository*：處理文字標記與螢光筆功能
6. *progress-repository*：追蹤用戶閱讀進度
7. *task-repository*：管理每日任務記錄與完成狀態

*資料庫採用 SQLite 作為單一儲存解決方案*，透過 better-sqlite3 進行連線管理。這種設計大幅簡化了開發與部署的複雜性，避免了多資料庫架構的技術障礙，同時保持了邏輯上的資料分層概念。SQLite 資料庫包含所有業務資料，包括用戶帳戶、學習進度、筆記內容、社群討論與每日任務記錄。

系統整合兩項關鍵外部 AI 服務以增強功能性：

1. *Perplexity Sonar API*：提供具備網路搜尋能力的 AI 問答服務，讓用戶在閱讀時可以即時詢問與紅樓夢相關的問題，並獲得包含學術文獻引用的專業回答。

2. *OpenAI GPT-5-mini*：負責所有學習任務的 AI 評分，包括閱讀理解、詩詞賞析、人物分析、文化知識測驗與評論詮釋等五種任務類型的智能評估與回饋生成。
