...
> 本文件記錄需要在 2025/12/13 前完成的七項關鍵任務
...
- **Work Description**:
    - Why:
        1. [x] 目前圖譜頁面進行放大縮小操作後，會重新回到初始位置，無法正常瀏覽
        2. [x] 節點持續抖動，影響視覺體驗
        3. [SKIP] 介面需要參照範例設計進行優化，使其更簡潔明瞭
    - How:
...
## Phase 7: 閱讀頁面工具欄標題置中問題修復

### [x] **Task ID**: TASK-007
- **Task Name**: 閱讀頁面上方工具欄標題文字未對齊畫面中央問題修復
- **Work Description**:
    - Why:
        1. 閱讀頁面上方工具欄的章節標題（如「第一回 甄士隱夢幻識通靈 賈雨村風塵懷閨秀」）沒有對齊畫面的水平中央
        2. 左側區域（選回、設定按鈕）與右側區域（筆記、日曆、AI、全螢幕、1x2 等圖示按鈕）寬度不對稱
        3. 導致中間的標題文字視覺上偏移，影響整體 UI 美觀和專業感
    - How:
        1. 分析現有工具欄的 CSS 佈局結構（可能是 flexbox 或 grid）
        2. 採用以下其中一種解決方案：
            - **方案 A**：使用 CSS Grid 三欄佈局，左右欄固定寬度相等，中間欄自動置中
            - **方案 B**：使用 flexbox + absolute positioning，標題使用絕對定位置中
            - **方案 C**：左右區域使用相同的固定寬度（取較大者），確保對稱
        3. 確保響應式設計在不同螢幕寬度下仍保持標題置中
        4. 測試各種章節標題長度下的顯示效果
- **Resources Required**:
    - Materials: 工具欄元件、CSS 樣式檔
    - Personnel: 前端開發
    - Reference Codes/docs:
        - `src/app/(main)/read-book/page.tsx` - 閱讀頁面
        - `src/components/reading/` - 閱讀頁面相關元件
        - `temp/20251205_UI_problem.jpg` - 問題截圖參考
- **Deliverables**:
    - [ ] 工具欄標題文字水平置中於畫面
    - [ ] 左右區域視覺對稱（或透過佈局技巧達成置中效果）
    - [ ] 響應式設計正常運作
    - [ ] 長標題與短標題都能正確顯示
    - [ ] 不影響現有工具欄按鈕的功能
- **Dependencies**: 無
...
- **Notes**:
    - 問題截圖顯示：
        - 左側：「選回」、「設定」兩個按鈕
        - 中間：章節標題「第一回 甄士隱夢幻識通靈 賈雨村風塵懷閨秀」
        - 右側：筆記、日曆、AI、全螢幕、1x2 等多個圖示按鈕
    - 建議採用 CSS Grid 方案，因為較容易維護且不需要 JavaScript 計算
...
| TASK-007 | 閱讀頁面工具欄標題置中修復 | 低 | ⬜ 未開始 | 無 |

## 建議執行順序

1. **TASK-006**[x] - AI 問答功能修復（最高優先，核心功能）
2. **TASK-003**[x] - 圖譜頁面載入錯誤修復（TASK-005 的前置條件）
3. **TASK-002**[x] - 閱讀頁面文本顯示問題修復（高優先，使用者體驗）
4. **TASK-004**[x] - 登入後首頁設定（中優先）
5. **TASK-001**[x] - 帳戶設置功能修復（中優先）
6. **TASK-005**[x] - 圖譜頁面互動與介面優化（依賴 TASK-003）
7. **TASK-007**[x] - 閱讀頁面工具欄標題置中修復（低優先，UI 微調）
