...
2. **TASK-003**[PENDING] - 圖譜頁面載入錯誤修復（TASK-005 的前置條件）
3. **TASK-002** - 閱讀頁面文本顯示問題修復（高優先，使用者體驗）
4. **TASK-004**[Jack] - 登入後首頁設定（中優先）
5. **TASK-001**[Jack] - 帳戶設置功能修復（中優先）
6. **TASK-005**[PENDING] - 圖譜頁面互動與介面優化（依賴 TASK-003）
7. **TASK-007**[Jack] - 閱讀頁面工具欄標題置中修復（低優先，UI 微調）
