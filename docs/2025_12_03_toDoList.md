...
- **Estimated completed time**: 6-8 小時
- **Work Description**:
    - Why: AI 問答區域存在多個問題（參見 `temp/perplexityAI_Bug/bug_2025_11_29.png`）
    - **2025-11-29 回報問題**:
...
- **Completion Status**: ⚠️ **需重新修復** (2025-11-29 發現新問題)
...
- **Completion Status**: ⚠️ **需重新修復** (2025-11-29 發現新問題，原標示「修復完成，待驗證」)
- **Notes**: 需參考 D:\AboutCoding\SideProject\TextReader\githubRepo\epub.js 的實作方式

---

### [ ] **Task 4.6**: Console 執行時錯誤修復 (多項)
...
- **Work Description**:
    ...
    - **錯誤清單**:
        1. **用戶 Profile 錯誤**: `Cannot read properties of undefined (reading 'getTime')` - 導致無法載入經驗值、等級資料
           ```
           ...
           ```
        2. **每日進度錯誤**: `q.getProgress is not a function` - 影響每日任務進度顯示
           ```
           Error fetching daily progress: TypeError: q.getProgress is not a function
           at w.getUserDailyProgress (.next/server/chunks/9813.js:1:3913)
           ```
        3. **OpenAI API 超時**: `fetch failed - connect ETIMEDOUT` - API 呼叫超時問題
           ```
...
### 高優先 (2025-11-29 更新)
1. **Task 4.6** - Console 執行時錯誤修復 (影響多項核心功能) - **新增**
2. **Task 4.2** - AI 問答回覆功能修復 (核心功能) - ⚠️ **需重新修復**
3. **Task 4.7** - 登入後首頁顯示「尚未登入」- **新增**
4. **Task 4.5** - 雙欄閱讀功能修復 - ⚠️ **需重新修復**

### 中優先
5. **Task 4.8** - 註冊帳號時允許輸入用戶名稱 - **新增**
6. **Task 4.9** - 紅學社筆記編輯後分享內容未同步更新 - **新增**
7. **Task 3.1** - 任務顯示 Bug 修復 - 修復完成
8. **Task 1.3** - 主頁 UI 佈局修復 - 修復完成
