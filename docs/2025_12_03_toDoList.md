...
- **Work Description**:
    ...
    - **2025-12-02 回報問題 (新)**:
        1. 回答時僅顯示「正在深度思考」，不會 streaming 顯示內容
        2. 無法看到正式輸出內容
- **Resources Required**:
    - Materials: `temp\perplexityAI_Bug\aiQA_problem03.jpg` 錯誤截圖
    - Personnel: 全端開發者 1 名
    - Reference Codes/docs:
        - `src/components/ui/ConversationFlow.tsx` - 對話流程元件
...
- **Work Description**:
    ...
    - **錯誤清單**:
        1. **每日進度錯誤 (2025-11-30 確認仍存在)**: `q.getProgress is not a function` - 影響每日任務進度顯示，導致 AI 無法持續生成題目
           ```
           2025-11-30 07:44:59.481 [error] Error fetching daily progress: TypeError: q.getProgress is not a function
            ...
            ai 無法生成題目
           ```
        2. **經驗值問題**: AI 批改後 (明明是95分) 未成功加 EXP 經驗值
        3. **任務完成狀態**: 完成任務後介面未顯示標示為完成
    - How:
...
### [ ] (skip) **Task 4.9**: 紅學社筆記編輯後分享內容未同步更新
...
### 高優先 (2025-11-30 更新)
1. [continue] **Task 4.2** - AI 問答回覆功能修復 (2025-11-30 發現 UI 完全無法顯示) - ⚠️ **需重新修復**
2. [continue] **Task 4.5** - 雙欄閱讀功能修復 (2025-11-30 確認仍未解決) - ⚠️ **需重新修復**

### 已修復完成
- **Task 4.1** - AI 問答對話歷史保存 - 修復完成
- **Task 4.7** - 登入後首頁顯示「尚未登入」
- **Task 4.6** - Console 執行時錯誤修復 (影響多項核心功能，包含 `getProgress` 錯誤)
...
- **Task 4.10** - 閱讀頁面筆記公開分享未出現在紅學社 - 修復完成
- **Task 2.2** - 學習進度總覽動態數據
- **Task 3.2** - 預生成題目系統

### 可延後 (時間允許再處理)
1.  **Task 2.1** - 虛擬居所功能實作
2.  [continue] **Task 4.4** - 補充章回內容至 20 回
3.  **Task 3.3** - 計畫介面設計
